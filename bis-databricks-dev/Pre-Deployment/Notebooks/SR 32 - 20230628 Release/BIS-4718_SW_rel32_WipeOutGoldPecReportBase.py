...
def truncateTable(clientcode):
  print("Truncating table")
  trunSql = f"""TRUNCATE TABLE {clientcode}.gold_ma_pecreportbase"""

  #run trucnate sql
  spark.sql(trunSql)

  nextTrunSql = f"""TRUNCATE TABLE {clientcode}.gold_ma_cdialertline"""
  #run trucnate sql
  spark.sql(nextTrunSql)
...
for database in databases: 
  ...
  try:
    ...
    goldMemberPath = f"/mnt/{clientcode}/Gold/MA/PEC/pecReportBase"
    if(path_exists(goldMemberPath)):
      truncateTable(clientcode)
