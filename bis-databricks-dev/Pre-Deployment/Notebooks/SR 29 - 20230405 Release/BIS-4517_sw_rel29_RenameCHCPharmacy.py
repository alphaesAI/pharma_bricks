...
def updateColumns(path):
  ...
  newDf = oldDf.withColumn("UniquePersonKey",col("UniquePersonID"))\
                .withColumn("NDCCodeSTD",col("NDC1"))\
                .withColumn("UnitsOfMeasure",col("ND1UOM"))\
                .withColumn("Units",col("NDC1Units"))\
                .withColumn("ProductID", substring(col("ProductID"),1,10))
  
  columns_to_drop = ["UniquePersonID", "NDC1", "ND1UOM", "NDC1Units"]
  newDf = newDf.drop(*columns_to_drop)
  
  newDf.write.format("delta").mode("overwrite").option("mergeSchema","true").save(path)
...
for c in clients: 
  ...
  try:
    ...
    oldPath = fr"dbfs:/mnt/{clientCode}/consolidated/ACA/Data/CHCPharmacyClaim"
    newPath = fr"dbfs:/mnt/{clientCode}/consolidated/ACA/Data/Pharmacy"

    if(path_exists(oldPath)):
      dbutils.fs.mv(oldPath, newPath, True)
      updateColumns(newPath)
