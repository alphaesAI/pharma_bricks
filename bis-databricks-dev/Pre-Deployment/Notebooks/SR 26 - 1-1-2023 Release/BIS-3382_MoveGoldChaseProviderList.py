...
# Copy chaseproviderlist from Risk to client and then delete from Risk. we could use just move command, but it might leave in bad state in case stops in middle
...
for database in databases: 
  ...
  try:
    ...
    srcChaseProviderListPath = f"/mnt/{clientcode}/Gold/MA/Risk/ChaseProviderList"
    destChaseProviderListPath = f"/mnt/{clientcode}/Gold/MA/Client/ChaseProviderList"
    if(path_exists(srcChaseProviderListPath) and not path_exists(destChaseProviderListPath)):
      dbutils.fs.cp(srcChaseProviderListPath,destChaseProviderListPath,True)
      delete_mounted_dir(srcChaseProviderListPath)
