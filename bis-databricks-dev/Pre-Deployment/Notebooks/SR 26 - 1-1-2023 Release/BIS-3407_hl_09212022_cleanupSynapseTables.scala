...
SELECT distinct TABLE_SCHEMA + '.' + TABLE_NAME as tableName  into #tempdelete
FROM INFORMATION_SCHEMA.TABLES
WHERE
CASE
    WHEN TABLE_NAME LIKE 'gold[_]%' THEN 1
    WHEN TABLE_NAME LIKE 'platinum[_]%' THEN 1
    WHEN TABLE_NAME IN('GoldenClaim','GoldenClaimHistory') THEN 0
    WHEN TABLE_NAME LIKE 'gold%' THEN 2
    WHEN TABLE_NAME LIKE 'dim%' THEN 2
    WHEN TABLE_NAME LIKE 'fact%' THEN 2
    WHEN TABLE_NAME LIKE 'databricks_streaming_checkpoint_%' THEN 2
    ELSE 0
END =2
...
CASE
    WHEN TABLE_NAME LIKE 'gold[_]%' THEN 1
    WHEN TABLE_NAME LIKE 'platinum[_]%' THEN 1
    WHEN TABLE_NAME IN('GoldenClaim','GoldenClaimHistory') THEN 0   --- keep GoldenClaim tables
    WHEN TABLE_NAME LIKE 'gold%' THEN 2
    WHEN TABLE_NAME LIKE 'dim%' THEN 2
    WHEN TABLE_NAME LIKE 'fact%' THEN 2
    WHEN TABLE_NAME LIKE 'databricks_streaming_checkpoint_%' THEN 2
