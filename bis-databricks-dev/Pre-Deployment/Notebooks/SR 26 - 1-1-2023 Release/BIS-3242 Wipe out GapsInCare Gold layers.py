...
def path_exists(pathToCheck): 
  ...
  IsExists = fs.exists(sc._jvm.org.apache.hadoop.fs.Path(gapsInCarePath))
  return IsExists 

# COMMAND ----------

# MAGIC %fs ls 'mnt/devidap1/Gold/MA/Quality'
...
for database in databases: 
  ...
  try:
    ...
    gapsInCarePath = f"/mnt/{clientcode}/Gold/MA/Quality/GapsInCare"
    if(path_exists(gapsInCarePath)):
      delete_mounted_dir(gapsInCarePath)
    gapsInCareRRPath = f"/mnt/{clientcode}/Gold/MA/Quality/GapsInCareRR"
    if(path_exists(gapsInCareRRPath)):
      delete_mounted_dir(gapsInCareRRPath)
