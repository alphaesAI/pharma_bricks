DECLARE @GCCount INTEGER = 0

IF EXISTS(SELECT 0 
			FROM INFORMATION_SCHEMA.TABLES 
			WHERE TABLE_SCHEMA = 'devidap1' 
			AND TABLE_NAME = 'tmp_ms_xx_GoldenClaim')
BEGIN
DROP TABLE devidap1.tmp_ms_xx_GoldenClaim
END

IF EXISTS(SELECT 1 FROM sys.columns 
          WHERE Name = N'IsDMEClaim'
          AND Object_ID = Object_ID(N'devidap1.GoldenClaim'))
BEGIN
	PRINT 'Old Structure Found For GoldenClaim.  Beginning Update'

	--Check for data 
	SELECT @GCCount = COUNT(1) FROM devidap1.GoldenClaim
	...
	BEGIN
		...
		BEGIN
			CREATE TABLE devidap1.GoldenClaim_Temp	
			(
		...
		TRUNCATE TABLE devidap1.GoldenClaim_Temp

		INSERT INTO devidap1.GoldenClaim_Temp
		...
		FROM devidap1.GoldenClaim
	END

/*	--Reset the GCH table by dropping and recreating
	DROP TABLE devidap1.GoldenClaim

	CREATE TABLE devidap1.GoldenClaim	
	(
		GeneratedGoldenClaimsUniqueKey varchar(255) NOT NULL  PRIMARY KEY NONCLUSTERED(GeneratedGoldenClaimsUniqueKey ASC) NOT ENFORCED,
		GeneratedMedicalClaimsUniqueKey varchar(255) NOT NULL , 
		ClientID           varchar(20)  NOT NULL,
